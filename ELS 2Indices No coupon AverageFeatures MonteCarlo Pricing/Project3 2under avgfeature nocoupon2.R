...
for(i in 1:nt){
   for(l in 1:nsim){
    ...
    if(i == nt){
      ...
      if(und[,l]>= K1) result[i,l] <- (1000+1000*((und[,l]/1000-1.21)*3.34)+415)
      if((und[,l] < K1) & (und[,l] >= K3)) result[i,l] <- (1000+1000*((und[,l]/1000-1)*1.5)+100)
      if((und[,l] < K3) & (und[,l] >= K5)) result[i,l] <- (1000+1000*((und[,l]/1000-0.95)*2))
      if(und[,l] < K5) result[i,l] <- (1000*(und[,l]/1000)+50)
    }
...
result2 <- result
for(i in 1:nsim){
  if(result2[nt,i] > 2116.6){ # max value
    result2[nt,i] <- 2116.6}
  if(result2[nt,i] < 50){ # min value
    result2[nt,i] <- 50}
}


ans<-sum(result2[nt,])/nsim*exp(-rf*t)
print(ans)
