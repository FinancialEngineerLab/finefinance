...
barrier_CRR <- function(S0,k,h,v,r,div,T,nmb_p){
  ...
  for(i in (nt-1):1){
    for (j in (nt-1):1){
      if ((st[i,j] < 95)&(cumsum(dt)==c(0.04, 0.08,0.12,0.16))){
        opt[i,j] <- 0
      }
      else{
...
error_result <- result-5.6711051343
error_result

plot(error_result, type='l', col='black',
     main = "C-DO Barrier Option error (BS-CRR)", ylab = "Results", xlab="Steps", xlim=c(0,1000))
...
for(n in 50:1000){
  for (i in 2:n){
    ...
    d <- 1/u
    s1 <- 100 * (u ^ i) * (d ^ (n - i))
    if ((s1 > 95)&(T/n == c(0.04,0.08,0.12,0.16){
      sk <- s1
