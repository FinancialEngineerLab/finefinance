...
term_2 <- function(s,vol,mat=1/12,r=0.02, long_short = 'short', stock_path, underlying_amount = 100000){
  ...
  vega <- as.numeric(GBSGreeks("Vega", "c",s, x,mat,r,0, vol))
  vega_path <- c(vega)
  ...
  for (z in c(2:length(stock_path))) {
    ...
    Call_Value <- as.numeric(GBSOption("c", S, x, mat, r, 0, vol*0.85)@price)
    option_path <- c(option_path, Call_Value)
    
    vega <- numeric(GBSGreeks("Vega", "c",S, x, time_left,r,0, vol))
