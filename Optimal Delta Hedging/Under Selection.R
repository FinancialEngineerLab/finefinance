...
#date change!!!!!!!!!!!!!!!!!!!!!!!!!
for (startdate in seq(as.Date('2015-07-13'), as.Date('2015-08-11'), by = "day")){
  
  
  ### stock code change as you want
  stock <- `086790.KS`[,'086790.KS.Close']
  stock_ret <- na.omit(Delt(stock))
  stock_real <- stock[index(stock) >= startdate & index(stock) <= as.Date('2015-08-11') + (365*3),]
  stock_ret_hist <- stock_ret[index(stock_ret) >= '2018-01-11' & index(stock_ret) <= '2018-07-12',]
...
row.names(reg_long_table) <- seq.Date(as.Date('2015-07-13'), as.Date('2015-08-11'), by = "day")
reg_long_table <- as.data.frame(reg_long_table)
row.names(reg_short_table) <- seq.Date(as.Date('2015-07-13'), as.Date('2015-08-11'), by = "day")
