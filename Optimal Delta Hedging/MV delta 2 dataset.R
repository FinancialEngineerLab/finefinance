...
term_2 <- function(s,vol,mat=0.5,r=0.02, long_short = 'short', stock_path, underlying_amount = 10){
  ...
  delta <- ifelse(long_short == 'short',
                  as.numeric(GBSGreeks("Delta", "c",s, x, mat,r,0, vol*1.15)),
                  as.numeric(GBSGreeks("Delta", "c",s, x, mat,r,0, vol*0.85)))
  delta_path <- c(delta)
  
  vega <- ifelse(long_short == 'short',
                 as.numeric(GBSGreeks("Vega", "c",s, x, mat,r,0, vol*1.15)),
                 as.numeric(GBSGreeks("Vega", "c",s, x, mat,r,0, vol*0.85)))
  vega_path <- c(vega)
  ...
  share_purchased <- ifelse(long_short == "short", mindelta * underlying_amount,
                            -mindelta * underlying_amount)
  ...
  for (z in c(2:length(stock_path))) {
    ...
    delta <-  ifelse(long_short == 'short',
                     as.numeric(GBSGreeks("Delta", "c",S, x, time_left,r,0, vol*1.15)),
                     as.numeric(GBSGreeks("Delta", "c",S, x, time_left,r,0, vol*0.85)))
    ...
    vega <- ifelse(long_short == 'short',
                   as.numeric(GBSGreeks("Vega", "c",S, x, time_left,r,0, vol*1.15)),
                   as.numeric(GBSGreeks("Vega", "c",S, x, time_left,r,0, vol*0.85)))
    ...
    # GBSVol
    mindelta <- ifelse(S != Stock_path[z-1],as.numeric(delta + vega*(((data_coef$Intercept[z-1]/((S-stock_path[z-1])*vega)*sqrt(time_left)*S+data_coef$beta1[z-1]*delta-data_coef$beta2[z-1]*delta*delta))/(sqrt(time_left)*(S)))),
                       mindelta)
    mindelta <- ifelse(z == length(stock_path), delta,mindelta)
    mindelta_path <- c(mindelta_path, mindelta)
    
    share_purchased <- ifelse(long_short == 'short', (mindelta - mindelta_path[z-1]) * underlying_amount
                              ,-(mindelta - mindelta_path[z-1]) * underlying_amount)
...
result <- term_2(s =  stockprice, vol = stock_vol_hist, stock_path = stock_real, long_short = "long", underlying_amount = 10)
